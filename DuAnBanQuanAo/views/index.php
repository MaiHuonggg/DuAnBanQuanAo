    ...
    switch ($act) {
        ...
        case 'addtocart':
            if (isset($_POST['addtocart']) && ($_POST['addtocart'])) {
                ...
                for($i = 0; $i < count($carts); $i++){
                    if($carts[$i][0] == $id && strcmp($carts[$i][5], $size) == 0 && strcmp($carts[$i][6], $color) == 0){
                        $check = $i;
                    }
                }
                if($check != -1){
                    $carts[$check][4] = $carts[$check][4] + 1;
                    $thanhtien = $carts[$check][4] * $giamoi;
                    $_SESSION['mycart'] = $carts;
                    include "../views/giohang.php";
                    break;
                }else {
                    $soluong = $_POST['soluong'];
                    $thanhtien = $soluong * $giamoi;
                    $spadd = [$id, $name, $img, $giamoi, $soluong, $size, $color, $thanhtien];
        ...
        case 'hoadon':
            ...
            foreach ($_SESSION['mycart'] as $cart) {
                insert_cart($_SESSION['user']['id'], $cart[0], $cart[2], $cart[1], $cart[3], $cart[4], $cart[7], $cart[5], $cart[6], $idbill);
            }
            //xóa session cart
            $_SESSION['cart'] = [];
            // }   
